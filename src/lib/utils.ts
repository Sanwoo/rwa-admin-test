...
export function isActivePathname(basePathname: string, currentPathname: string, exactMatch: boolean = false) {
  ...
  return currentPathname.startsWith(basePathname) && (currentPathname.length === basePathname.length || currentPathname[basePathname.length] === '/')
}
...
// Check if the current environment is production(Compatible with Vercel)
export const isProductionInVercel = () => {
  return process.env.NODE_ENV === 'production' && process.env.NEXT_PUBLIC_VERCEL_ENV === 'production'
}

export const getFactoryAddress = () => {
  return isProductionInVercel() ? process.env.NEXT_PUBLIC_FACTORY_ADDRESS_BSC : process.env.NEXT_PUBLIC_FACTORY_ADDRESS_BSC_TESTNET
}

export const getWEUSDAddress = () => {
  return isProductionInVercel() ? process.env.NEXT_PUBLIC_WEUSD_ADDRESS_BSC : process.env.NEXT_PUBLIC_WEUSD_ADDRESS_BSC_TESTNET
}
