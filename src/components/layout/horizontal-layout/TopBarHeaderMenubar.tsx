...
export function TopBarHeaderMenubar() {
  ...
  const renderMenuItem = (item: NavigationRootItem | NavigationNestedItem) => {
    ...
    if ('href' in item) {
      ...
      return (
        <MenubarItem asChild>
          <Link href={item.href} className={cn('w-full gap-2', isActive && 'bg-accent')}>
            {'iconName' in item ? <DynamicIcon name={item.iconName} className="h-4 w-4" /> : <DynamicIcon name="Circle" className="h-2 w-2" />}
            <span>{item.title}</span>
            {'label' in item && <Badge variant="secondary">{item.label}</Badge>}
          </Link>
        </MenubarItem>
